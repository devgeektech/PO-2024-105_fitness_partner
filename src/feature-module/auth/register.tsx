...
import {
  ARGENTINA_NATIONALITY,
  ISTUPNICA_OR_BRISOVNICA_NATIONALITY,
} from "../../constants";
...
const parentDetailSchema: any = Yup.object().shape({
  ...
  bic: Yup.string()
    .min(8, LANG.MINIMUM_LIMIT_BIC_CHAR)
    .max(11, LANG.MAXIMUM_LIMIT_BIC_CHAR)
    .required(LANG.FIELD_IS_REQUIRED),
...
const subscriptionSchema = Yup.object().shape({
  ...
  privacyPolicy: Yup.bool()
    .oneOf([true], LANG.FIELD_IS_REQUIRED)
    .required(LANG.FIELD_IS_REQUIRED),
  dataPolicy: Yup.bool()
    .oneOf([true], LANG.FIELD_IS_REQUIRED)
    .required(LANG.FIELD_IS_REQUIRED),
      ...
      try {
        const payload: any = {
          ...
          siblingDetails: JSON.stringify(
            subscriptionFormik.values.siblingDetails
          ),
        };
  ...
  const renderLayout = (activeStep: number) => {
    switch (activeStep) {
      ...
      case 2: {
        return (
          <SecondStep
            formik={parentDetailFormik}
            registerValue={formik.values}
          />
        );
      }
      case 3: {
      ...
      case 4: {
        return (
          <FourthStep formik={uploadFormik} registerValue={formik.values} />
        );
      }
      case 5: {
